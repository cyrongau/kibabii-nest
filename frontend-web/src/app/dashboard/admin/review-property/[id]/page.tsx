...
function DetailBox({ label, value }: { label: string, value: string }) {
  return (
    <div className="p-6 bg-muted/20 rounded-2xl border border-border-subtle shadow-soft-sm group hover:border-primary/20 transition-all">
      <div className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] mb-1.5 leading-none">System Data</div>
  ...
  if (isLoading) {
    return (
      <div className="p-12 flex flex-col items-center justify-center h-[70vh] gap-8 animate-in fade-in duration-1000">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/40 italic">Loading property details...</p>
      </div>
    );
  }
    ...
    <div className="p-8 lg:p-12 max-w-7xl mx-auto space-y-12 bg-background min-h-screen">
      <header className="flex flex-col md:flex-row items-center justify-between gap-10">
        <div className="flex items-center gap-8 w-full md:w-auto">
          ...
          <div className="space-y-1 group">
            <h1 className="text-4xl font-black text-foreground tracking-tighter group-hover:text-primary transition-colors">Review Property</h1>
            <p className="text-muted-foreground font-medium max-w-2xl leading-relaxed text-sm mt-1.5 flex items-center gap-3">
              <span className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse shadow-glow"></span>
              Conduct a thorough review of the property listing details before granting approval for the Kibabii Nest platform.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 w-full md:w-auto">
          ...
          >
            <XCircle size={20} /> Revoke Approval
          </button>
          <button 
            disabled={isProcessing || property.verified}
          ...
          >
            ...
            <CheckCircle2 size={20} className="relative z-10" /> <span className="relative z-10">Approve Property</span>
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
          <section className="p-12 rounded-[3.5rem] border border-border-subtle shadow-soft-2xl space-y-10 relative bg-card overflow-hidden group transition-colors">
            ...
            <div className="flex flex-col sm:flex-row items-start justify-between gap-8 relative z-10">
              ...
              <span className={`px-6 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-[0.3em] shadow-soft-sm border border-white/5 flex items-center gap-3 ${property.verified ? 'badge-emerald' : 'badge-orange'}`}>
                ...
                {property.verified ? 'Approved' : 'Pending Review'}
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 relative z-10">
              <DetailBox label="Property Category" value={property.category?.name || 'Standard'} />
              <DetailBox label="Monthly Rent" value={`Ksh ${property.price?.toLocaleString()}`} />
              <DetailBox label="Distance to Campus" value={`${property.distanceToCampus}m away`} />
            </div>
            ...
            <div className="relative z-10 bg-muted/10 p-8 rounded-[2.5rem] border border-border-subtle">
              <h3 className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                <div className="w-6 h-1 bg-primary rounded-full"></div> Property Description
              </h3>
              <p className="text-muted-foreground leading-relaxed font-bold text-sm">{property.description}</p>
            </div>
          </section>

          <section className="p-12 rounded-[3.5rem] border border-border-subtle shadow-soft-2xl space-y-10 relative bg-card group transition-colors">
             <div className="flex items-center gap-5 relative z-10">
               <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20">
                 <ImageIcon className="text-primary" size={28} />
               </div>
               <div>
                 <h3 className="text-2xl font-black text-foreground tracking-tighter">Property Images</h3>
                 <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] mt-1.5 italic text-sm">Images provided by the property owner</p>
               </div>
             </div>
        ...
        <div className="space-y-12">
           <section className="p-12 rounded-[3.5rem] border border-border-subtle shadow-soft-2xl space-y-10 relative bg-card group transition-colors">
             ...
             <div className="flex items-center gap-5 relative z-10">
               ...
               <div>
                 <h3 className="text-2xl font-black text-foreground tracking-tighter">Landlord Details</h3>
                 <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] mt-1.5 italic text-sm">Owner Contact Details</p>
               </div>
             </div>

             <div className="flex items-center gap-6 p-6 bg-muted/30 rounded-[2.5rem] border border-border-subtle relative z-10 group/owner-card hover:border-primary/20 transition-all">
                <div className="w-16 h-16 bg-foreground text-background rounded-2xl overflow-hidden shadow-soft-sm flex items-center justify-center font-black text-2xl group-hover/owner-card:scale-110 group-hover/owner-card:bg-primary group-hover/owner-card:text-white transition-all duration-500 relative border border-white/10">
                  {property.landlord?.avatar ? (
                    <img src={property.landlord.avatar} alt="" className="w-full h-full object-cover" />
                  ) : (
                    property.landlord?.name ? property.landlord?.name[0].toUpperCase() : 'L'
                  )}
                </div>
                <div>
                  <div className="text-lg font-black text-foreground group-hover/owner-card:text-primary transition-colors">{property.landlord?.name}</div>
             ...
             <div className="space-y-6 px-4 relative z-10">
                ...
                <ContactItem icon={<Phone size={20} />} value={property.landlord?.phone || 'PRIVATE DATA'} />
             </div>
           ...
           <section className="bg-primary/5 p-12 rounded-[3.5rem] border border-primary/10 space-y-6 relative overflow-hidden group">
              <div className="text-[10px] font-black text-primary uppercase tracking-[0.3em] flex items-center gap-3">
                <div className="w-2 h-2 bg-primary rounded-full animate-pulse"></div> Verification Guidelines
              </div>
              <p className="text-xs font-bold text-muted-foreground/60 leading-relaxed">By approving this property, you confirm it meets all institutional and platform safety standards for student accommodation.</p>
           </section>
        </div>
      </div>
    </div>
